...
const App = () => {
    ...
    // 📍 เพิ่ม 'documents' ใน currentView
    ...
    // 📍 [ข้อ 3] State สำหรับ Quick Add ใน Modal แอดมิน
    const [quickAddType, setQuickAddType] = useState('job'); // 'job', 'leave', 'event', 'holiday'
    ...
    // 📍 [ข้อ 4] ฟังก์ชันสวิตช์เอกสารที่ง่ายที่สุด
    ...
    const handleBookingSubmit = async (e) => {
        ...
        // 📍 [ข้อ 3] ประมวลผลเมื่อส่งฟอร์มจาก Quick Add (แท็บวันลา / กิจกรรม / วันหยุด)
        if (quickAddType !== 'job') {
            ...
            if(ok) { setModal(null); showSlideToast('บันทึกสำเร็จ', 'success'); }
            return;
        }
    ...
    // 📍 [ข้อ 2] สำหรับฟอร์มแก้ไขรายการพิเศษโดยเฉพาะ
    const handleEditSpecialSubmit = async (e) => {
        e.preventDefault();
        const fd = new FormData(e.target);
        const newTitle = fd.get('site_name');
        const ok = await apiAction({ action: 'update_booking', id: modal.data.id, site_name: newTitle, user: user.username }, 'กำลังอัปเดต...');
        if(ok) { setModal(null); showSlideToast('อัปเดตสำเร็จ', 'success'); }
    };
    ...
    return (
        <div className="app-container" onClick={() => setShowParticipantDropdown(false)}>
            ...
            {/* 📍 [ข้อ 4] อัปเดตเมนูด้านล่าง เพิ่มแท็บ "เอกสาร" (ถ้าเป็นแอดมิน) */}
                                            ...
                                            {!d.isEmpty && (db.inspectors || []).map((ins, idx) => {
                                                ...
                                                return (
                                                    <div key={idx} 
                                                        ...
                                                        onClick={() => {
                                                            if (!isAdmin && (d.isGlobalHoliday || d.isGlobalEvent || hasLeave)) return; 
                                                        ...
                                                        {cellTasks.map((task, tIdx) => {
                                                            ...
                                                            // 📍 [ข้อ 1] ยกเลิก AI อ่านคำ กลับมาใช้รูปแบบเดิม (เฉพาะงานที่มาจากระบบจริงถึงจะเขียว)
                                                            const isLeaveCard = task.job_type === 'leave' || String(task.equipment_no || '').toLowerCase().startsWith('leave_');
                                                            const isEventCard = task.job_type === 'company_event' || String(task.equipment_no || '').toLowerCase().startsWith('event_');
                                                            ...
                                                            return (
                                                                <div key={task.id || tIdx} 
                                                                    ...
                                                                    onClick={(e) => {
                                                                        e.stopPropagation(); 
                                                                        if (!isAdmin && (isLeaveCard || isEventCard)) return; // คนทั่วไปกดวันลานิ่งสนิท
                                                                        if (isLeaveCard || isEventCard) {
                                                                            setConfirmDialog({msg: 'ต้องการลบการตั้งค่านี้ใช่หรือไม่?', onConfirm: () => { setConfirmDialog(null); apiAction({action: 'delete_booking', id: task.id, user: user.username}, 'กำลังลบ...'); }});
            ...
            {/* 📍 [ข้อ 4] หน้าต่างตรวจสอบเอกสาร (Doc Checker) แบบสวิตช์เปิดปิด */}
                                ...
                                <div className="space-y-3">
                                    ...
                                    {(leaveStartDate && leaveEndDate && leaveInspector) && (
                                        <div className="p-3 bg-white border border-yellow-300 rounded-lg text-center shadow-sm">
                                            ...
                                            {leaveMins && <div className="text-xs text-yellow-600 font-bold mt-1 bg-yellow-50 p-1.5 rounded">เวลา: {formatDuration(leaveMins)}/วัน <br/>(รวม: {formatDuration(leaveDates.length * leaveMins)})</div>}
                                        </div>
                                    )}
                                ...
                                <div className="space-y-3">
                                    ...
                                    {(eventStartDate && eventEndDate) && (
                                        <div className="p-3 bg-white border border-green-300 rounded-lg text-center shadow-sm">
                                            ...
                                            {eventMins && <div className="text-xs text-green-600 font-bold mt-1 bg-green-50 p-1.5 rounded">เวลา: {formatDuration(eventMins)}/วัน <br/>(รวม: {formatDuration(eventDates.length * eventMins)})</div>}
                                        </div>
                                    )}
            ...
            {/* 📍 [ข้อ 3] Modal จองคิวงาน และ Quick Add สำหรับ Admin */}
                ...
                <div className="backdrop z-[150]">
                    <div className="modal-card">
                        ...
                        <div className="p-6 overflow-y-auto">
                            ...
                            {modal.type === 'booking' ? (
                                <>
                                    {/* 📍 [ข้อ 3] Quick Add Selector เฉพาะ Admin เมื่อกดจิ้มช่องในปฏิทิน */}
                                    {isAdmin && !modal.data?.id && !modal.data?.isAdminOverride && (
                                        <div className="quick-add-tabs">
                                            <button className={`quick-tab ${quickAddType === 'job' ? 'active' : ''}`} onClick={() => setQuickAddType('job')}>งานตรวจ</button>
                                            <button className={`quick-tab ${quickAddType === 'leave' ? 'active' : ''}`} onClick={() => setQuickAddType('leave')}>วันลา</button>
                                            <button className={`quick-tab ${quickAddType === 'event' ? 'active' : ''}`} onClick={() => setQuickAddType('event')}>กิจกรรม</button>
                                            <button className={`quick-tab ${quickAddType === 'holiday' ? 'active' : ''}`} onClick={() => setQuickAddType('holiday')}>วันหยุด</button>
                                        </div>
                                    )}

                                    <form onSubmit={handleBookingSubmit} className="space-y-3">
                                        {/* ฟอร์มของ Quick Add ชนิดอื่นๆ ที่ไม่ใช่ "งานตรวจ" */}
                                            ...
                                            <div className="p-3 bg-slate-50 border border-slate-200 rounded-xl space-y-3">
                                                ...
                                                {quickAddType !== 'holiday' && (
                                                    <div className="grid grid-cols-2 gap-2">
                                                        ...
                                                        <div><label className="text-xs font-bold text-slate-700">เวลาสิ้นสุด</label><input type="time" name="end_time" className="text-sm p-2 rounded border w-full"/></div>
                                                    </div>
                                                )}
            ...
            {/* 📍 [ข้อ 2] Modal เฉพาะสำหรับแก้ไขรายการพิเศษ (วันลา/กิจกรรม/วันหยุด) */}
