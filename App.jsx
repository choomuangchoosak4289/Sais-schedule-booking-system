...
const App = () => {
    ...
    // 📍 [ข้อ 3] เปลี่ยนระบบ Toast ให้เป็น Modal สีเขียวกลางจอ
    ...
    const handleBulkDelete = async (type, ids) => {
        setConfirmDialog({
            ...
            onConfirm: async () => {
                ...
                await fetchData();
                setLoadingMsg(null);
                if(allOk) setSuccessModal('ลบข้อมูลที่เลือกสำเร็จแล้ว'); else setAlertMsg('ลบข้อมูลสำเร็จบางส่วน');
            }
        });
    };
    ...
    useEffect(() => {
        ...
        if(SCRIPT_URL) load();
        const timer = setInterval(() => { if (!modal && !showLogin && !showActivityModal && !alertMsg && !confirmDialog && !promptDialog && !loadingMsg && !successModal && SCRIPT_URL) fetchData(); }, 180000); 
    ...
    const handleBookingSubmit = async (e) => {
        ...
        // 📍 [ข้อ 2] จับเวลาที่กรอกมา ใส่เข้าไปในชื่อโครงการ
    ...
    return (
        <div className="app-container" onClick={() => setShowParticipantDropdown(false)}>
            {/* 📍 [ข้อ 3] ลบ Toast ออก เอา Success Modal มาแทน */}
            {successModal && (
                <div className="backdrop z-[700]">
                    <div className="bg-white w-[85%] max-w-[320px] rounded-3xl p-6 text-center shadow-2xl animate-pop">
                        ...
                        <h3 className="text-lg font-bold text-slate-800 mb-2">สำเร็จ</h3>
                        <p className="text-sm text-slate-600 mb-6">{successModal}</p>
                        <button onClick={() => setSuccessModal(null)} className="w-full py-3 bg-green-600 text-white rounded-xl font-bold shadow-md">ตกลง</button>
                    </div>
                </div>
            )}
                                                ...
                                                return (
                                                    <div key={idx} 
                                                        ...
                                                        {cellTasks.map((task, tIdx) => {
                                                            ...
                                                            // 📍 [ข้อ 1] ล็อกสีเขียวให้กิจกรรม 100%
                                                            ...
                                                            else if (isEventCard) cardTypeClass = 'card-type-event'; // บังคับเขียว
                                ...
                                <>
                                    {(db.notifications || []).filter(n => n.target === user?.username || (isAdmin && n.target === 'ALL_ADMIN')).map((n, i) => (
                                        <div key={i} onClick={() => markNotifAsRead(n.id)} className={`p-3 rounded-xl border cursor-pointer transition-all ${String(n.isRead) === 'true' ? 'bg-slate-50 border-slate-200 opacity-70' : 'bg-blue-50 border-blue-200 shadow-sm'}`}>
                                            ...
                                            {/* 📍 [ข้อ 5] จัดการ Timezone ตรงนี้ */}
                                            <div className="text-[9px] text-slate-400 mt-1 text-right">{new Date(n.timestamp).toLocaleString('th-TH', { timeZone: 'Asia/Bangkok', year: 'numeric', month: 'short', day: 'numeric', hour: '2-digit', minute:'2-digit' })}</div>
                                        </div>
                                    ))}
                                    ...
                                    {(db.logs || []).slice(0, logsLimit).map((log, i) => (
                                        <div key={i} className="bg-slate-50 p-3 rounded-xl border border-slate-200 shadow-sm">
                                            <div className="flex justify-between text-[10px] font-bold text-slate-400 mb-1">
                                                ...
                                                <span>{new Date(log.timestamp).toLocaleString('th-TH', { timeZone: 'Asia/Bangkok', month: 'short', day: 'numeric', hour: '2-digit', minute:'2-digit' })}</span>
                                            </div>
            ...
            {/* 📍 [ข้อ 2] เพิ่มเวลาในฟอร์มจองคิวงานปกติ */}
                                    ...
                                    <form onSubmit={handleBookingSubmit} className="space-y-3">
                                        {quickAddType !== 'job' && isAdmin && !modal.data?.id && !modal.data?.isAdminOverride ? (
                                            ...
                                            <>
                                                ...
                                                {/* 📍 [ข้อ 2] เพิ่มตัวเลือกกำหนดเวลาสำหรับฟอร์มงานปกติ */}
            ...
            {/* 📍 [ข้อ 4] Modal สำหรับแก้ไขรายการพิเศษ (วันลา/กิจกรรม) เพิ่มเปลี่ยนคนได้ */}
