...
const App = () => {
    ...
    // 📍 ระบบจัดการแผนที่แบบปลอดภัย ป้องกันจอขาว
    ...
    const handleBookingSubmit = async (e) => {
        ...
        // 📍 [ข้อ 1 แก้หน้าจอขาว] ใช้ระบบตรวจสอบที่ปลอดภัย 100%
        ...
        const isDup = (db.bookings || []).some(b => {
            ...
            // 📍 ห้าม Eq No เดียวกัน จองซ้ำวันเดียวกัน (ไม่ว่าจะเป็นแอดมินหรือใคร)
            if (String(b.equipment_no) === String(data.equipment_no)) return true;
            // ถ้าไม่ใช่แอดมิน ห้ามจองคิวซ้ำลงผู้ตรวจคนเดิมในวันนั้น
        ...
        <div className="app-container">
                                    ...
                                    return (
                                        <React.Fragment key={index}>
                                            ...
                                            {!d.isEmpty && (db.inspectors || []).map((ins, idx) => {
                                                ...
                                                // 📍 [ข้อ 6] เช็คว่าคนนี้มีคิวลาในวันนี้หรือไม่
                                                ...
                                                return (
                                                    <div key={idx} 
                                                        ...
                                                        {/* 📍 [ข้อ 6] ซ่อนกิจกรรมบริษัท ถ้าคนๆ นั้นมีการลา (แสดงว่าวันลาสำคัญกว่ากิจกรรม) */}
                                                        {d.isGlobalEvent && !hasLeave && d.globalEvents.map((ge, gei) => <div key={'ge'+gei} className="holiday-label-new">{ge.site_name}</div>)}
                                                        
                                                        {/* 📍 [ข้อ 4] ลบคำว่า HOLIDAY ออกในวันอาทิตย์ (มีแค่แถบแดง) */}
                                                        ...
                                                        {cellTasks.map((task, tIdx) => {
                                                            ...
                                                            return (
                                                                <div key={task.id || tIdx} 
                                                                    ...
                                                                    {/* 📍 [ข้อ 3] ฟอนต์ปรับขนาดและห่อข้อความอัตโนมัติ ไม่ล้นขอบ */}
            ...
            {currentView === 'admin' && isAdmin && (
                <div className="page-view">
                        ...
                        <div className="animate-pop pb-10">
                            <div className="flex justify-between items-center mb-4">
                                <button onClick={() => {
                                    ...
                                    // 📍 [แก้หน้าจอขาว] สร้างคิวแทนส่ง isAdminOverride ไปชัวร์ๆ
                    ...
                    {/* 📍 [ข้อ 2] หน้าจัดการวันลา เพิ่มลิสต์รายการให้ลบได้ */}
                    {adminTab === 'leaves' && (
                        <div className="animate-pop space-y-4 pb-10">
                                ...
                                <div className="space-y-3">
                                    ...
                                    {(leaveStartDate && leaveEndDate && leaveInspector) && (
                                        <div className="p-3 bg-white border border-yellow-300 rounded-lg text-center shadow-sm">
                                            <div className="text-xs font-bold text-yellow-700 mb-1">จำนวนวันลาสุทธิ (ไม่รวมวันอาทิตย์/หยุด)</div>
                            ...
                            <div className="bg-white p-4 rounded-xl shadow-sm border border-yellow-200 mt-4">
                                <h3 className="font-bold text-yellow-800 mb-3 border-b border-yellow-100 pb-2">รายการวันลาที่ตั้งไว้ (แตะ X เพื่อลบ)</h3>
                                <div className="space-y-2 max-h-[30vh] overflow-y-auto pr-1">
                                    {(db.bookings || []).filter(b => b.job_type === 'leave' && String(b.status) !== 'cancelled').sort((a, b) => new Date(b.date) - new Date(a.date)).map((h, i) => (
                                        <div key={i} className="flex justify-between items-center bg-yellow-50 p-2 rounded-lg border border-yellow-100">
                                            ...
                                            <button onClick={() => handleCancelBooking(h)} className="text-red-500 p-1.5 bg-white rounded shadow-sm"><Icons.X /></button>
                                        </div>
                                    ))}
                    ...
                    {/* 📍 [ข้อ 2] หน้าจัดการกิจกรรม เพิ่มลิสต์รายการให้ลบได้ */}
                    {adminTab === 'events' && (
                        <div className="animate-pop space-y-4 pb-10">
                            <form onSubmit={async (e) => {
                            ...
                            <div className="bg-white p-4 rounded-xl shadow-sm border border-green-200 mt-4">
                                <h3 className="font-bold text-green-800 mb-3 border-b border-green-100 pb-2">รายการกิจกรรมที่ตั้งไว้ (แตะ X เพื่อลบ)</h3>
                                <div className="space-y-2 max-h-[30vh] overflow-y-auto pr-1">
                    ...
                    {/* 📍 [ข้อ 2] หน้าจัดการวันหยุดนักขัตฤกษ์ เพิ่มลิสต์รายการให้ลบได้ */}
                    {adminTab === 'holidays' && (
                        <div className="animate-pop space-y-4 pb-10">
                            <form onSubmit={async (e) => {
                                e.preventDefault(); const fd = new FormData(e.target);
                            ...
                            <div className="bg-white p-4 rounded-xl shadow-sm border border-red-200 mt-4">
                                <h3 className="font-bold text-red-800 mb-3 border-b border-red-100 pb-2">รายการวันหยุดที่ตั้งไว้ (แตะ X เพื่อลบ)</h3>
                                <div className="space-y-2 max-h-[30vh] overflow-y-auto pr-1">
                        ...
                        <div className="p-6 overflow-y-auto">
                            ...
                            {modal.type === 'booking' ? (
                                <form onSubmit={handleBookingSubmit} className="space-y-3">
                                    ...
                                    {isAdmin && modal?.data?.isAdminOverride === true && (
                                ...
                                <div className="space-y-4">
                                    ...
                                    {liveMapUrl && <div><span className="text-xs font-bold text-slate-500 uppercase">Location</span><div className="map-preview relative mt-1 bg-slate-100 rounded-xl overflow-hidden border"><iframe width="100%" height="100%" frameBorder="0" src={liveMapUrl} loading="lazy"></iframe></div></div>}
