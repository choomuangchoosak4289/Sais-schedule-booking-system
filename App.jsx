...
const App = () => {
    ...
    const [currentView, setCurrentView] = useState('calendar'); // 📍 views: calendar, search, my_bookings, admin
    ...
    // 📍 ฟังก์ชันกรองพื้นที่ใช้ทั้งหน้าปฏิทิน และ หน้าค้นหา
    ...
    const handleBookingSubmit = async (e) => {
        ...
        // 📍 [ข้อ 1 แก้จอขาว] ใช้งานอย่างปลอดภัย 100% 
        const isFromAdminPanel = isAdmin && currentView === 'admin' && adminTab === 'bookings' && !modal.data?.id;
    ...
    return (
        <div className="app-container">
            ...
            {/* 📍 [ข้อ 6] ปรับปรุงเมนูด้านล่างสุด (Bottom Nav) เพิ่มปุ่มค้นหา และปุ่มออกระบบให้ Admin */}
                            ...
                            <div className="calendar-grid" style={{ '--col-count': (db.inspectors || []).length || 1 }}>
                                ...
                                {(db.inspectors || []).map((ins, i) => (
                                    <div key={i} className="sticky-top">
                                        {/* 📍 [ข้อ 2] เอาตัวย่อออก เหลือแต่ชื่อผู้ตรวจ และบีบตารางขึ้น */}
                                        <div className="font-bold truncate w-full text-center px-1 text-[11px] leading-tight py-1">{ins.name || '-'}</div>
                                    </div>
                                ))}
                                            ...
                                            {!d.isEmpty && (db.inspectors || []).map((ins, idx) => {
                                                ...
                                                return (
                                                    <div key={idx} 
                                                        ...
                                                        onClick={() => {
                                                            // 📍 [ข้อ 2, 3] แอดมินสามารถคลิกช่องว่างหรือพื้นที่ว่างในเซลล์เพื่อจองงาน/กิจกรรมซ้อนได้
                                                        ...
                                                        {/* 📍 [ข้อ 4] ลบคำว่า HOLIDAY ออกในวันอาทิตย์ */}
                                                        ...
                                                        {cellTasks.map((task, tIdx) => {
                                                            ...
                                                            return (
                                                                <div key={task.id || tIdx} 
                                                                    ...
                                                                    onClick={(e) => {
                                                                        e.stopPropagation(); // กันคลิกทะลุหลังการ์ด
                                                                        if (task.job_type === 'leave' || task.job_type === 'company_event') {
                                                                    ...
                                                                    {/* 📍 [ข้อ 5] ฟอนต์พอดีกับการ์ด ไม่ดันตาราง */}
            ...
            {/* 📍 [ข้อ 6] สร้างหน้าต่างค้นหาแยกต่างหาก (พร้อม Filter พื้นที่) */}
            {currentView === 'search' && (
                        ...
                        {searchQuery.trim() === '' ? (
                            ...
                            (() => {
                                const searchResults = (db.bookings || []).filter(b => {
                                    ...
                                    // ประยุกต์ใช้ Filter พื้นที่ร่วมด้วย
                                    if (filterArea !== 'All' && String(b.area || '') !== filterArea) return false;
                        ...
                        <div className="animate-pop pb-10">
                            <div className="flex justify-between items-center mb-4">
                                <button onClick={() => {
                                    ...
                                    // 📍 [ข้อ 1 แก้จอขาว] ระบุว่าการเปิด Modal ครั้งนี้มาจากปุ่มสร้างคิวงานแทน
                                ...
                                <div className="space-y-3">
                                    ...
                                    <div>
                                        ...
                                        <select name="inspector" required className="bg-white p-2 rounded-lg border border-green-200 w-full text-sm font-bold outline-none">
                                            {/* 📍 [ข้อ 7] เปลี่ยนคำเป็น ผู้ตรวจทุกคน */}
                        ...
                        <div className="p-6 overflow-y-auto">
                            ...
                            {modal.type === 'booking' ? (
                                <form onSubmit={handleBookingSubmit} className="space-y-3">
                                    ...
                                    {/* 📍 [แก้หน้าจอขาว] ใช้ optional chaining กับ modal.data */}
                                    {isAdmin && currentView === 'admin' && adminTab === 'bookings' && !modal.data?.id && (
