...
// 📍 CalendarGrid ฉบับแสดงผลครบ 100% ไม่ซ่อนการ์ด และจัดสีตาม Requirement
                ...
                return (
                    <React.Fragment key={index}>
                        ...
                        {!d.isEmpty && (db.inspectors || []).map((ins, idx) => {
                            // ดึงงานทั้งหมด ไม่มีการใช้ slice(0,3) เพื่อให้การ์ดแสดงผลทั้งหมดเรียงลงมา
                            ...
                            return (
                                <div key={idx} 
                                    ...
                                    {cellTasks.map((task, tIdx) => {
                                        let cardTypeClass = 'card-type-new'; 
                                        ...
                                        // 📍 เงื่อนไขการเช็คประเภทงานแบบครอบคลุม
                                        const isLeaveCard = jobType === 'leave' || combinedStr.includes('leave_') || combinedStr.includes('ลา');
                                        const isEventCard = jobType === 'company_event' || combinedStr.includes('event_') || combinedStr.includes('meeting') || combinedStr.includes('office') || combinedStr.includes('อบรม') || combinedStr.includes('s&q') || combinedStr.includes('family');
                                        const isHolidayCard = jobType === 'public_holiday' || combinedStr.includes('hld_');
                                        const isUpcountry = area !== '' && area !== 'กรุงเทพและปริมณฑล' && area !== 'ไม่ระบุ';

                                        // 📍 ลำดับความสำคัญของสีตาม Requirement
                                        if (isHolidayCard) cardTypeClass = 'card-type-holiday'; // วันหยุด (แดง)
                                        else if (isEventCard) cardTypeClass = 'card-type-event'; // วันกิจกรรม (เขียว)
                                        else if (isLeaveCard) cardTypeClass = 'card-type-leave'; // วันลา (เหลืองเข้ม)
                                        else if (isUpcountry) cardTypeClass = 'card-type-upcountry'; // ต่างจังหวัด (ชมพู)
                                        else if (jobType === 'mod') cardTypeClass = 'card-type-mod'; // MOD (เทาเข้ม)
                                        else if (jobType.includes('re-ins') || jobType.includes('temporary') || jobType.includes('builder lift')) cardTypeClass = 'card-type-reins'; // Re-ins (เหลืองอ่อน)
                                        else cardTypeClass = 'card-type-new'; // New / กทม. (สีเทา)

                                        return (
                                            <div key={task.id || tIdx} 
                                                draggable={isAdmin && !isLeaveCard && !isEventCard && !isHolidayCard} 
                                                onDragStart={(e) => handleDragStart(e, task.id)} 
                                                className={`task-content ${cardTypeClass} relative overflow-hidden`}
                                                onClick={(e) => {
                                                    e.stopPropagation(); 
                                                ...
                                                {isLeaveCard || isEventCard || isHolidayCard ? (
                                                    <div className="special-event-text auto-text z-10 relative px-1">{task.site_name}</div>
                                                ) : (
                                                    <div className="z-10 relative px-1 w-full">
                                                        <div className="text-line-1 auto-text leading-tight">{task.equipment_no} <span className="opacity-70">/</span> {task.unit_no}</div>
                                                        <div className="text-line-2 auto-text leading-tight mt-0.5">{task.site_name}</div>
                                                    </div>
                                                )}
                                            </div>
                                        );
