...
// 📍 ฟังก์ชันจัดการสีการ์ด (Inline Styles) ตัดปัญหา CSS ตีกัน 100%
const getCardStyle = (task) => {
    ...
    // ลำดับ Priority ของสีตาม Requirement
    ...
    if (area !== '' && area !== 'กรุงเทพและปริมณฑล' && area !== 'ไม่ระบุ') return { bg: '#f472b6', text: '#ffffff', isSpecial: false }; // ต่างจังหวัด = ชมพู
    if (jobType === 'mod') return { bg: '#64748b', text: '#ffffff', isSpecial: false }; // MOD = เทาเข้ม
    if (jobType.includes('re-ins') || jobType.includes('temporary') || jobType.includes('builder lift')) return { bg: '#fef08a', text: '#854d0e', isSpecial: false }; // Re-ins = เหลืองอ่อน
    return { bg: '#e2e8f0', text: '#1e293b', isSpecial: false }; // New และ กทม. = เทา (ค่าเริ่มต้น)
};

// 📍 CalendarGrid (ยกระดับความเร็วด้วย Lookup Map)
const CalendarGrid = React.memo(({ daysInView, db, isAdmin, user, setShowLogin, setModal, setAlertMsg, handleDrop, handleDragOver, handleDragLeave, handleDragStart, setConfirmDialog, apiAction, setQuickAddType, filteredBookings }) => {
    ...
    // 📍 อัปเกรดความเร็วระดับ O(1) : จัดกลุ่มงานไว้ล่วงหน้า เพื่อไม่ให้วนลูปค้นหาซ้ำซ้อนตอนเรนเดอร์ตาราง
                                    ...
                                    {cellTasks.map((task, tIdx) => {
                                        ...
                                        return (
                                            <div key={task.id || tIdx} 
                                                ...
                                                {/* แถบสีด้านซ้ายตาม Product Line ของงาน */}
                                                {!styleObj.isSpecial && (
...
const App = () => {
    ...
    // 📍 3. ตัวแปรเก็บการตั้งค่าขนาดฟอนต์ 3 กลุ่ม
