...
const App = () => {
    ...
    const [currentView, setCurrentView] = useState('calendar'); // 📍 มุมมอง: calendar, search, my_bookings, admin
    ...
    // 📍 ฟังก์ชันกรองเฉพาะคิวงานของปฏิทิน
    ...
    const handleBookingSubmit = async (e) => {
        ...
        // 📍 ใช้งานอย่างปลอดภัย (ใช้ Optional Chaining `?.` แก้อาการจอขาว)
        const targetInspector = modal.data?.isAdminOverride ? fd.get('admin_inspector_target') : modal.data?.inspector_name;
        const targetDate = modal.data?.isAdminOverride ? fd.get('admin_date_target') : modal.data?.date;
    ...
    return (
        <div className="app-container">
            ...
            {/* 📍 [ข้อ 6] เพิ่มปุ่มค้นหางานเป็น Tab ด้านล่าง */}
            <div className="bottom-nav">
                <div className={`nav-item ${currentView === 'calendar' ? 'active' : ''}`} onClick={() => setCurrentView('calendar')}><Icons.Home /> ปฏิทินจอง</div>
                <div className={`nav-item ${currentView === 'search' ? 'active' : ''}`} onClick={() => setCurrentView('search')}><Icons.Search /> ค้นหางาน</div>
            ...
            {currentView === 'calendar' && (
                <div className="grid-container">
                    <div className="filter-bar">
                        {/* เอาช่อง Search ออก เหลือแค่ Filter พื้นที่ */}
                        <div className="flex-1"></div>
                        <select className="text-xs border border-slate-300 rounded-lg p-2 bg-white outline-none w-28 shadow-sm font-bold text-slate-600" value={filterArea} onChange={(e) => setFilterArea(e.target.value)}>
                                            ...
                                            {!d.isEmpty && (db.inspectors || []).map((ins, idx) => {
                                                ...
                                                return (
                                                    <div key={idx} 
                                                        ...
                                                        onClick={() => {
                                                            // 📍 [ข้อ 2 & 3] เปลี่ยนเงื่อนไข ไม่ Block วันหยุดถ้าเป็นแอดมิน, ให้คลิกช่องว่างเพื่อจองคิวทับซ้อนได้
                                                        ...
                                                        {/* 📍 [ข้อ 4] เอาคำว่า Holiday วันอาทิตย์ออก */}
                                                        
                                                        {/* 📍 [ข้อ 5] กำหนดการ์ดไม่ให้เสียทรงเมื่อตัวหนังสือยาว */}
            ...
            {/* 📍 [ข้อ 6] หน้าต่างค้นหาแบบใหม่ */}
            {currentView === 'search' && (
                <div className="page-view relative">
                    <div className="sticky top-0 bg-f1f5f9 z-10 pb-4 pt-2">
                        <h2 className="text-xl font-bold text-slate-800 mb-4 flex items-center gap-2"><Icons.Search /> ค้นหางาน</h2>
                    ...
                    <div className="space-y-3 pb-10">
                        {searchQuery.trim() === '' ? (
                            <div className="text-center text-slate-400 p-8 border-2 border-dashed border-slate-200 rounded-xl mt-4">พิมพ์ข้อมูลเพื่อเริ่มค้นหา...</div>
                        ) : (
                            (() => {
                                const searchResults = (db.bookings || []).filter(b => {
                                    ...
                                    return String(b.equipment_no || '').toLowerCase().includes(s) || 
                                           String(b.site_name || '').toLowerCase().includes(s) || 
                                           String(b.inspector_name || '').toLowerCase().includes(s) ||
                                           String(b.area || '').toLowerCase().includes(s);
                        ...
                        <div className="p-6 overflow-y-auto">
                            ...
                            {modal.type === 'booking' ? (
                                <form onSubmit={handleBookingSubmit} className="space-y-3">
                                    ...
                                    {isAdmin && adminTab === 'bookings' && !modal.data?.id && (
