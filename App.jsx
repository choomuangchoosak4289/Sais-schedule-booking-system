...
const App = () => {
    ...
    // State สำหรับ Pop-up ระบบทั้งหมด (แทนที่ window.confirm และ window.prompt)
    const [alertMsg, setAlertMsg] = useState(null);
    const [confirmDialog, setConfirmDialog] = useState(null);
    const [promptDialog, setPromptDialog] = useState(null); // 📍 Pop-up ให้กรอกข้อความใหม่
    ...
    // 📍 จัดการยกเลิกคิวงานด้วย Custom Prompt Modal
    ...
    // 📍 ฟังก์ชันย้ายคิวด้วย Drag & Drop ธีมใหม่
    const handleDrop = async (e, targetDate, targetInspector) => {
        ...
        const isDup = db.bookings.some(b => String(b.date).split('T')[0] === targetDate && String(b.inspector_name) === targetInspector && b.id !== taskId && !String(b.equipment_no).startsWith('LEAVE_') && !String(b.equipment_no).startsWith('EVENT_'));
    ...
    }, [db.bookings, searchQuery, filterArea]);
    ...
    const daysInView = useMemo(() => {
        ...
        for (let i = 0; i < 16; i++) {
            ...
            if (d <= end) {
                ...
                const globalHoliday = (db.bookings || []).find(b => b.date && String(b.date).split('T')[0] === localDateStr && String(b.inspector_name) === 'SYSTEM_HOLIDAY');
                const globalEvent = (db.bookings || []).find(b => b.date && String(b.date).split('T')[0] === localDateStr && String(b.inspector_name) === 'SYSTEM_EVENT');
                days.push({ 
                    full: localDateStr, day: d, weekday: date.toLocaleDateString('en-US', { weekday: 'short' }), 
                    isSunday: date.getDay() === 0, 
                    isGlobalHoliday: !!globalHoliday || date.getDay() === 0, holidayData: globalHoliday || null,
                    isGlobalEvent: !!globalEvent, eventData: globalEvent || null,
    ...
    if (!SCRIPT_URL) {
        return <div className="h-screen w-full flex items-center justify-center flex-col gap-4 p-8 text-center"><div className="text-4xl text-red-500"><Icons.Alert /></div><h2 className="text-xl font-bold text-slate-800">เกิดข้อผิดพลาด</h2><p className="text-slate-600 text-sm">ไม่พบการตั้งค่าเชื่อมต่อฐานข้อมูล (URL) กรุณาตรวจสอบไฟล์ <code>config.js</code> หรือล้างแคชเบราว์เซอร์</p></div>;
    }
    ...
    return (
        <div className="app-container">
                                    ...
                                    return (
                                        <React.Fragment key={index}>
                                            ...
                                            {!d.isEmpty && (db.inspectors || []).map((ins, idx) => {
                                                ...
                                                const hasTask = !!task && String(task.inspector_name) !== 'SYSTEM_HOLIDAY' && String(task.inspector_name) !== 'SYSTEM_EVENT';
                                                ...
                                                return (
                                                    <div key={idx} 
                                                        ...
                                                        onClick={() => {
                                                            if (hasTask) { 
                                                                if (task.job_type === 'leave' || task.job_type === 'company_event') {
                                                                    if(isAdmin) {
                                                                        setConfirmDialog({
                                                                            msg: 'ต้องการลบการตั้งค่าวันลา/กิจกรรมนี้?', 
                                                                            onConfirm: () => { setConfirmDialog(null); apiAction({action: 'delete_booking', id: task.id, user: user.username}, 'กำลังลบ...'); }
                                                                        });
                                                                    }
                                                                } else { setModal({ type: 'detail', data: task }); }
                                                            } else {
                                                                if ((d.isGlobalHoliday || d.isGlobalEvent) && !isAdmin) return setAlertMsg('วันหยุด/กิจกรรมระบบ ไม่เปิดให้จองคิวครับ');
                                                        ...
                                                        {!hasTask && d.isGlobalHoliday && <div className="holiday-label-new">{d.holidayData ? d.holidayData.site_name : (d.isSunday ? '' : 'HOLIDAY')}</div>}
                                                        {!hasTask && d.isGlobalEvent && <div className="holiday-label-new">{d.eventData ? d.eventData.site_name : 'กิจกรรม'}</div>}
                                                        
                                                        {hasTask && (
                                                            <div draggable={isAdmin && task.job_type !== 'leave' && task.job_type !== 'company_event'} onDragStart={(e) => handleDragStart(e, task.id)} className={`task-content ${cardTypeClass} ${areaClass}`}>
                                                                {task.job_type === 'leave' || task.job_type === 'company_event' ? (
                                                                    <div className="text-xs font-bold w-full truncate px-1">{task.site_name}</div>
                                                                ) : (
                                                                    <>
                ...
                <div className="page-view">
                    ...
                    {adminTab === 'leaves' && (
                        <div className="animate-pop space-y-4 pb-10">
                            <form onSubmit={async (e) => {
                                ...
                                const ok = await apiAction({ action: 'create_booking', date: fd.get('h_date'), inspector_name: fd.get('inspector'), job_type: 'leave', site_name: fd.get('leave_type'), equipment_no: `LEAVE_${Date.now()}`, user: user.username }, 'กำลังบันทึกวันลา...');
                                if(ok) { showSlideToast('เพิ่มวันลาสำเร็จ', 'success'); e.target.reset(); }
                                ...
                                <div className="space-y-3">
                                    <div><label className="text-xs font-bold text-yellow-700">วันที่</label><input type="date" name="h_date" required className="bg-white p-2 rounded-lg border border-yellow-200 w-full text-sm" /></div>
                                    <div>
                                        <label className="text-xs font-bold text-yellow-700">ผู้ตรวจ</label>
                                        <select name="inspector" required className="bg-white p-2 rounded-lg border border-yellow-200 w-full text-sm font-bold">
                                    ...
                                    <div>
                                        ...
                                        <select name="leave_type" className="bg-white p-2 rounded-lg border border-yellow-200 w-full text-sm font-bold">
                                            <option value="ลาพักร้อน">ลาพักร้อน</option><option value="ลาป่วย">ลาป่วย</option><option value="ลากิจ">ลากิจ</option>
                                        </select>
                                    </div>
                                    <button disabled={loadingMsg} className="w-full bg-yellow-600 text-white py-2 rounded-lg font-bold text-sm">บันทึกวันลา</button>
                                </div>
                            </form>
                        </div>
                    )}
                    ...
                    {adminTab === 'events' && (
                        <div className="animate-pop space-y-4 pb-10">
                            <form onSubmit={async (e) => {
                                e.preventDefault(); const fd = new FormData(e.target);
                                const target = fd.get('inspector');
                                const finalTarget = target === 'ALL' ? 'SYSTEM_EVENT' : target;
                                const ok = await apiAction({ action: 'create_booking', date: fd.get('h_date'), inspector_name: finalTarget, job_type: 'company_event', site_name: fd.get('event_name'), equipment_no: `EVENT_${Date.now()}`, user: user.username }, 'กำลังบันทึกกิจกรรม...');
                                if(ok) { showSlideToast('เพิ่มกิจกรรมสำเร็จ', 'success'); e.target.reset(); }
                                ...
                                <div className="space-y-3">
                                    <div><label className="text-xs font-bold text-green-700">วันที่</label><input type="date" name="h_date" required className="bg-white p-2 rounded-lg border border-green-200 w-full text-sm" /></div>
                                    <div>
                                        <label className="text-xs font-bold text-green-700">ผู้เข้าร่วม (ระบุทุกคน หรือ เฉพาะคน)</label>
                                        <select name="inspector" required className="bg-white p-2 rounded-lg border border-green-200 w-full text-sm font-bold">
                                            <option value="ALL">✅ ทุกคน (ทั้งบริษัท)</option>
                                            {(db.inspectors || []).map(i => <option key={i.name} value={i.name}>👤 เฉพาะ: {i.name}</option>)}
                                        </select>
                                    </div>
                                    <div><label className="text-xs font-bold text-green-700">ชื่อกิจกรรม</label><input type="text" name="event_name" required placeholder="เช่น อบรมความปลอดภัย" className="bg-white p-2 rounded-lg border border-green-200 w-full text-sm" /></div>
                                    <button disabled={loadingMsg} className="w-full bg-green-600 text-white py-2 rounded-lg font-bold text-sm">บันทึกกิจกรรม</button>
                                </div>
                            </form>
                        </div>
                    )}
                    ...
                    {adminTab === 'holidays' && (
                        <div className="animate-pop space-y-4 pb-10">
                            <form onSubmit={async (e) => {
                                e.preventDefault(); const fd = new FormData(e.target);
                                const ok = await apiAction({ action: 'create_booking', date: fd.get('h_date'), inspector_name: 'SYSTEM_HOLIDAY', job_type: 'public_holiday', site_name: fd.get('h_name'), equipment_no: `HLD_${Date.now()}`, user: user.username }, 'กำลังเพิ่มวันหยุด...');
                                ...
                                <div className="space-y-3">
                                    <div><label className="text-xs font-bold text-red-700">วันที่</label><input type="date" name="h_date" required className="bg-white p-2 rounded-lg border border-red-200 w-full text-sm" /></div>
                                    <div><label className="text-xs font-bold text-red-700">ชื่อวันหยุด</label><input type="text" name="h_name" required placeholder="เช่น วันสงกรานต์" className="bg-white p-2 rounded-lg border border-red-200 w-full text-sm" /></div>
                                    <button disabled={loadingMsg} className="w-full bg-red-600 text-white py-2 rounded-lg font-bold text-sm">บันทึกวันหยุด</button>
                                </div>
                            </form>
                        </div>
                    )}
            ...
            {/* 📍 ป๊อปอัปแจ้งเตือนแบบมีปุ่มเดียว (Custom Alert) */}
            {alertMsg && (
            ...
            {/* 📍 ป๊อปอัปยืนยันแบบมี 2 ปุ่ม (Custom Confirm) */}
            {confirmDialog && (
                <div className="backdrop z-[600]">
                    <div className="bg-white w-[85%] max-w-[320px] rounded-3xl p-6 text-center shadow-2xl animate-pop">
            ...
            {/* 📍 ป๊อปอัปสำหรับพิมพ์ข้อความ (Custom Prompt) - แทนที่ window.prompt */}
            {promptDialog && (
                <div className="backdrop z-[600]">
                    <div className="bg-white w-[85%] max-w-[320px] rounded-3xl p-6 text-center shadow-2xl animate-pop">
                        <div className="mx-auto w-14 h-14 bg-amber-50 text-amber-500 rounded-full flex items-center justify-center mb-4"><Icons.Alert /></div>
