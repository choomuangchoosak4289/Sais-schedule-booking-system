                                    ...
                                    {cellTasks.map((task, tIdx) => {
                                        ...
                                        return (
                                            <div key={task.id || tIdx} 
                                                draggable={isAdmin && !styleObj.isSpecial} 
                                                onDragStart={(e) => handleDragStart(e, task.id)} 
                                                className="task-content relative overflow-hidden"
                                                style={{ backgroundColor: styleObj.bg, color: styleObj.text }}
                                                onClick={(e) => {
                                                    e.stopPropagation(); 
                                                    if (!isAdmin && styleObj.isSpecial) return;
                                                    if (styleObj.isSpecial) {
                                                ...
                                                {styleObj.isSpecial ? (
                                                    <div className="special-event-text auto-text z-10 relative px-1 w-full">{task.site_name}</div>
                                                ) : (
                                                    <div className="z-10 relative px-1 pl-2 w-full">
                                                        <div className="text-eq auto-text leading-tight">{task.equipment_no} <span className="opacity-70">/</span> {task.unit_no}</div>
                                                        <div className="text-desc auto-text leading-tight mt-[1px]">{task.site_name}</div>
                                                    </div>
                                                )}
                                            </div>
                                        );
...
const App = () => {
    ...
    // ใช้ Ref เพื่อให้ setInterval อัปเดตข้อมูลได้ถูกต้องโดยไม่เกิด Stale Closure
            ...
            onConfirm: async () => {
                ...
                try {
                    ...
                    if (res.status === 'ok') {
                        ...
                        await fetchCoreData(true, null); // โหลดใหม่ทั้งหมด
    ...
    // 📍 1. ระบบ Stale-while-Revalidate (เปิดปุ๊บตารางขึ้นปั๊บ 0.1 วิ)
    useEffect(() => {
        const load = async () => {
            ...
            // เช็คว่ามีแคชไหม ถ้าไม่มีเลยแปลว่าเพิ่งโหลดครั้งแรก ให้ดึงอดีตมาด้วย
        ...
        const timer = setInterval(() => { 
            if (document.visibilityState === 'visible' && !modal && !showLogin && !showActivityModal && !alertMsg && !confirmDialog && !promptDialog && !loadingMsg && !successModal && SCRIPT_URL) {
                // อัปเดตราย 3 นาที: ดึงเฉพาะข้อมูลอนาคต (fetch_past: false) เพื่อลดภาระเน็ต
    ...
    // 📍 2. ฟังก์ชันผสานข้อมูลอดีต (Local) เข้ากับปัจจุบัน/อนาคต (Server)
    const fetchCoreData = async (needPast = false, currentCache = null) => {
        ...
        try {
            ...
            if (result.status === 'ok') {
                ...
                // ถ้านี่คือ Delta Sync (ไม่ได้ดึงอดีตมา) ให้ประกอบร่างอดีตจากเครื่องเข้ากับอนาคตจากเซิร์ฟเวอร์
                if (!needPast && currentCache && currentCache.bookings) {
                    const today = new Date();
                    today.setHours(0,0,0,0);
                    
                    // เตรียม List ID งานที่เพิ่งโหลดมาใหม่ เพื่อไม่ให้ชนกับงานในอดีต
                    const fetchedIds = new Set(result.data.bookings.map(b => b.id));

                    const pastCachedBookings = currentCache.bookings.filter(b => {
                        if (!b.date) return false;
                        const bDate = new Date(b.date);
                        // เก็บเฉพาะงานในอดีต ที่ไม่ได้ถูกส่งอัปเดตมาจาก Server ในรอบนี้
                        return bDate < today && !fetchedIds.has(b.id);
                    });
                    
                    // นำมาต่อกัน (อดีตอันแสนสงบ + อนาคตที่สดใหม่)
    ...
    // 📍 3. ดึงข้อมูลใหม่ยกชุดทันที เมื่อแอดมินหรือพนักงานกดเพิ่ม/ลบ/แก้ไขคิว
    const apiAction = async (payload, customLoadMsg = 'กำลังบันทึกข้อมูล...') => {
        ...
        try {
            ...
            setLoadingMsg(null);
            if (result.status === 'ok') { 
                await fetchCoreData(true, null); // บังคับ Full Sync เพื่อให้มั่นใจ 100%
    ...
    const handleToggleDoc = async (bookingId, docType, currentValue) => {
        ...
        // ทำ Optimistic Update บน UI ทันที
        setDb(prev => ({
            ...prev,
