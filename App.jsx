...
const App = () => {
    ...
    // 📍 1. State สำหรับระบบ "วันลาแบบคำนวณอัตโนมัติ"
    const [leaveStartDate, setLeaveStartDate] = useState('');
    const [leaveEndDate, setLeaveEndDate] = useState('');
    const [leaveInspector, setLeaveInspector] = useState('');
    const [leaveType, setLeaveType] = useState('ลาพักร้อน');

    // 📍 คำนวณจำนวนวันลาที่แท้จริง (หักวันอาทิตย์ วันหยุดนักขัตฤกษ์ และวันกิจกรรมบริษัท)
    const leaveDates = useMemo(() => {
        ...
        while (current <= end) {
            const localDateStr = utils.getLocalDateString(current);
            const isSunday = current.getDay() === 0;
            // เช็คว่าวันนี้เป็นวันหยุดระบบ หรือ กิจกรรมระบบ หรือไม่
            ...
            const isGlobalEvent = (db.bookings || []).some(b => b.date && String(b.date).split('T')[0] === localDateStr && String(b.inspector_name) === 'SYSTEM_EVENT');
            
            // ถัาไม่ใช่วันหยุด/กิจกรรม ถึงจะนับเป็นวันลา
            if (!isSunday && !isGlobalHoliday && !isGlobalEvent) {
        ...
        <div className="app-container">
            ...
            {currentView === 'admin' && isAdmin && (
                <div className="page-view">
                    ...
                    {/* 📍 [ข้อ 1] ฟอร์มตั้งค่าวันลา (ปรับปรุงคำนวณวันหักลบวันหยุด/กิจกรรมแล้ว) */}
                    {adminTab === 'leaves' && (
                        <div className="animate-pop space-y-4 pb-10">
                            <form onSubmit={async (e) => {
                                e.preventDefault(); 
                                if (leaveDates.length === 0) return setAlertMsg('ไม่มีวันลาที่สามารถตั้งค่าได้ (อาจตรงกับวันหยุดทั้งหมด หรือเลือกช่วงวันผิด)');
                                
                                setConfirmDialog({
                                    msg: `ยืนยันบันทึก ${leaveType} ให้ ${leaveInspector} จำนวน ${leaveDates.length} วัน?`,
                                    onConfirm: async () => {
                                        setConfirmDialog(null);
                                        const payload = { 
                                            action: 'create_multiple_bookings', 
                                            dates: leaveDates, 
                                            inspector_name: leaveInspector, 
                                            job_type: 'leave', 
                                            site_name: leaveType, 
                                            equipment_no: `LEAVE_${Date.now()}`, 
                                            user: user.username 
                                        };
                                        const ok = await apiAction(payload, 'กำลังบันทึกวันลา...');
                                        if(ok) { showSlideToast('เพิ่มวันลาสำเร็จ', 'success'); setLeaveStartDate(''); setLeaveEndDate(''); setLeaveInspector(''); }
                                    }
                                });
