...
const App = () => {
    ...
    // 📍 ระบบ Modal ยืนยัน / แจ้งเตือนแบบ Custom 100%
    ...
    const leaveDates = useMemo(() => {
        ...
        while (current <= end) {
            ...
            if (!isSunday && !isGlobalHoliday && !isGlobalEvent) dates.push(localDateStr);
    ...
    // 📍 ฟังก์ชัน Logout ใช้ Custom Modal
    const handleLogout = () => {
        setConfirmDialog({
    ...
    // 📍 ระบบจัดการการยกเลิกพร้อมเหตุผลผ่าน Custom Prompt
    ...
    // 📍 ลากและวางคิวงาน ใช้ Custom Modal
    ...
    if (!SCRIPT_URL) {
        return <div className="h-screen w-full flex items-center justify-center flex-col gap-4 p-8 text-center"><div className="text-4xl text-red-500"><Icons.Alert /></div><h2 className="text-xl font-bold text-slate-800">เกิดข้อผิดพลาด</h2><p className="text-slate-600 text-sm">ไม่พบการตั้งค่าเชื่อมต่อฐานข้อมูล (URL) กรุณาตรวจสอบไฟล์ <code>config.js</code></p></div>;
    }

    return (
        <div className="app-container">
            {/* 📍 ป๊อปอัปแจ้งเตือนด้านบน (Toast) */}
            {toast && (
            ...
            {/* 📍 หน้าจอโหลดแบบกระจกฝ้า */}
            {loadingMsg && (
                <div className="backdrop z-[700] gap-4">
                    <Icons.Loader />
                                    ...
                                    return (
                                        <React.Fragment key={index}>
                                            ...
                                            {!d.isEmpty && (db.inspectors || []).map((ins, idx) => {
                                                // 📍 ดึงข้อมูลทั้งหมดในช่องนี้ (.filter) เพื่อแสดงการ์ดหลายใบเรียงซ้อนกัน
                                                const cellTasks = filteredBookings.filter(b => b.date && String(b.date).split('T')[0] === d.full && String(b.inspector_name) === String(ins.name));
                                                ...
                                                return (
                                                    <div key={idx} 
                                                        ...
                                                        {d.isGlobalHoliday && d.globalHolidays.map((gh, ghi) => <div key={'gh'+ghi} className="holiday-label-new">{gh.site_name}</div>)}
                                                        {d.isGlobalEvent && d.globalEvents.map((ge, gei) => <div key={'ge'+gei} className="holiday-label-new" style={{background: 'rgba(0,100,0,0.2)'}}>{ge.site_name}</div>)}
                                                        {!hasTask && d.isSunday && !d.globalHolidays.length && <div className="holiday-label-new">HOLIDAY</div>}
                                                        
                                                        {/* 📍 เรนเดอร์การ์ดทั้งหมดที่มีในช่องนี้ */}
                                                        {cellTasks.map((task, tIdx) => {
                                                            ...
                                                            return (
                                                                <div key={task.id || tIdx} 
                                                                    ...
                                                                    onClick={(e) => {
                                                                        e.stopPropagation(); // กันคลิกทะลุช่องหลังการ์ด
                                                                        if (task.job_type === 'leave' || task.job_type === 'company_event') {
                                                                            if(isAdmin) {
                                                                                setConfirmDialog({
                                                                    ...
                                                                    {task.job_type === 'leave' || task.job_type === 'company_event' ? (
                                                                        <div className="text-[10px] font-bold w-full text-center px-1" style={{whiteSpace: 'normal'}}>{task.site_name}</div>
                                                                    ) : (
                                                                        <>
            ...
            {currentView === 'admin' && isAdmin && (
                <div className="page-view">
                    ...
                    {/* 📍 ฟอร์มตั้งค่าวันลา พร้อมระบุเวลา */}
                    {adminTab === 'leaves' && (
                            ...
                            <form onSubmit={async (e) => {
                                ...
                                setConfirmDialog({
                                    ...
                                    onConfirm: async () => {
                                        setConfirmDialog(null);
                                        const payload = { 
                                            action: 'create_multiple_bookings', dates: leaveDates, inspector_name: leaveInspector, 
                                            job_type: 'leave', site_name: finalLeaveName, equipment_no: `LEAVE_${Date.now()}`, user: user.username 
                                        };
                    ...
                    {/* 📍 ฟอร์มตั้งค่ากิจกรรมบริษัท พร้อมระบุเวลา */}
                    {adminTab === 'events' && (
                        <div className="animate-pop space-y-4 pb-10">
                            <form onSubmit={async (e) => {
                                ...
                                setConfirmDialog({
                                    ...
                                    onConfirm: async () => {
                                        setConfirmDialog(null);
                                        const payload = { 
                                            action: 'create_multiple_bookings', dates: eventDates, inspector_name: finalTarget, 
                                            job_type: 'company_event', site_name: eventName, equipment_no: `EVENT_${Date.now()}`, user: user.username 
                                        };
