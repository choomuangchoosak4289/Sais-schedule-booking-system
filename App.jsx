...
const App = () => {
    ...
    // 📍 [ข้อ 3] ฟังก์ชันคำนวณชั่วโมง
    const calculateHours = (start, end) => {
        if (!start || !end) return null;
        const [sh, sm] = start.split(':').map(Number);
        const [eh, em] = end.split(':').map(Number);
        let diff = (eh + em / 60) - (sh + sm / 60);
        if (diff < 0) diff += 24; // เผื่อกรณีข้ามคืน
    ...
    // 📍 [ข้อ 2] ฟังก์ชันจัดการ Checkbox กิจกรรมแบบเลือกหลายคน
        ...
        <div className="app-container">
            ...
            {currentView === 'admin' && isAdmin && (
                <div className="page-view">
                    ...
                    {adminTab === 'leaves' && (
                        <div className="animate-pop space-y-4 pb-10">
                            ...
                            <div className="bg-white p-4 rounded-xl shadow-sm border border-yellow-200 mt-4">
                                <h3 className="font-bold text-yellow-800 mb-3 border-b border-yellow-100 pb-2">รายการวันลาที่ตั้งไว้ (ระบบจัดการ)</h3>
                                <div className="space-y-2 max-h-[30vh] overflow-y-auto pr-1">
                                    {(db.bookings || []).filter(b => b.job_type === 'leave' && String(b.status) !== 'cancelled').sort((a,b)=> new Date(b.date) - new Date(a.date)).map((h, i) => (
                                        <div key={i} className="flex justify-between items-center bg-yellow-50 p-2 rounded-lg border border-yellow-100">
                                            ...
                                            <button onClick={() => { if(window.confirm('ต้องการลบการตั้งค่าวันลานี้?')) apiAction({action: 'delete_booking', id: h.id, user: user.username}, 'กำลังลบ...'); }} className="text-red-500 p-1.5 bg-white rounded shadow-sm border border-red-100"><Icons.X /></button>
                                        </div>
                                    ))}
                    ...
                    {/* 📍 [ข้อ 2] ฟอร์มตั้งค่ากิจกรรมแบบ Multi-select และ [ข้อ 3] คำนวณชั่วโมง */}
                    {adminTab === 'events' && (
                        <div className="animate-pop space-y-4 pb-10">
                            <form onSubmit={async (e) => {
                                ...
                                setConfirmDialog({
                                    msg: `ยืนยันบันทึกกิจกรรม ${eventName} ให้ ${eventParticipants.includes('ALL') ? 'ทุกคน' : eventParticipants.length + ' คน'} จำนวน ${eventDates.length} วัน?`,
                                    onConfirm: async () => {
                                        setConfirmDialog(null);
                                        let allSuccess = true;
                                        
                                        // ถ้ายกบริษัท ให้ยิง API เดียวเป็น SYSTEM_EVENT
                                        if (eventParticipants.includes('ALL')) {
                                            ...
                                            // ถ้าเลือกหลายคน ให้ยิง API วนลูปทีละคน
                                        ...
                                        if (allSuccess) { 
                                            ...
                                            setEventStartDate(''); setEventEndDate(''); 
                                            setEventStartTime(''); setEventEndTime('');
                                            setEventParticipants(['ALL']);
                                            e.target.reset(); 
                                        }
                                    }
                                });
                                ...
                                <div className="space-y-3">
                                    ...
                                    {/* 📍 [ข้อ 2] แผงเลือกผู้เข้าร่วมแบบ Multi-select */}
                                    <div className="bg-white p-3 rounded-lg border border-green-200">
                                        <label className="text-xs font-bold text-green-700 block mb-2 border-b pb-1">ผู้เข้าร่วม (เลือกได้มากกว่า 1 คน)</label>
                                        <div className="space-y-2">
                                            <label className="flex items-center gap-2 text-sm font-bold text-slate-800 bg-slate-50 p-2 rounded border">
                                            ...
                                            <div className="grid grid-cols-2 gap-2 pt-1">
                                                {(db.inspectors || []).map(i => (
                                                    <label key={i.name} className={`flex items-center gap-2 text-xs p-1.5 rounded border ${eventParticipants.includes('ALL') ? 'opacity-50 bg-slate-50' : (eventParticipants.includes(i.name) ? 'bg-green-50 border-green-300 font-bold text-green-800' : 'bg-white border-slate-200 text-slate-600')}`}>
                                                        <input type="checkbox" className="w-3.5 h-3.5 accent-green-600" checked={eventParticipants.includes(i.name) || eventParticipants.includes('ALL')} onChange={handleEventParticipantChange} value={i.name} disabled={eventParticipants.includes('ALL')} />
                                                        {i.name}
                                                    </label>
                                                ))}
                                            </div>
                                    ...
                                    {(eventStartDate && eventEndDate) && (
                                        <div className="p-3 bg-white border border-green-300 rounded-lg text-center shadow-sm">
                                            ...
                                            <div className="text-xl font-black text-green-600">
                                                ...
                                                {eventHours && <span className="text-sm font-bold ml-1 text-green-500">({eventHours} ชม./วัน)</span>}
                                            </div>
                            ...
                            <div className="bg-white p-4 rounded-xl shadow-sm border border-green-200 mt-4">
                                <h3 className="font-bold text-green-800 mb-3 border-b border-green-100 pb-2">รายการกิจกรรมที่ตั้งไว้ (ระบบจัดการ)</h3>
                                <div className="space-y-2 max-h-[30vh] overflow-y-auto pr-1">
                                    {(db.bookings || []).filter(b => b.job_type === 'company_event' && String(b.status) !== 'cancelled').sort((a,b)=> new Date(b.date) - new Date(a.date)).map((h, i) => (
                                        <div key={i} className="flex justify-between items-center bg-green-50 p-2 rounded-lg border border-green-100">
                                            ...
                                            <button onClick={() => { if(window.confirm('ต้องการลบการตั้งค่ากิจกรรมนี้?')) apiAction({action: 'delete_booking', id: h.id, user: user.username}, 'กำลังลบ...'); }} className="text-red-500 p-1.5 bg-white rounded shadow-sm border border-red-100"><Icons.X /></button>
                                        </div>
                                    ))}
                    ...
                    {adminTab === 'holidays' && (
                        <div className="animate-pop space-y-4 pb-10">
                            ...
                            <div className="bg-white p-4 rounded-xl shadow-sm border border-red-200 mt-4">
                                <h3 className="font-bold text-red-800 mb-3 border-b border-red-100 pb-2">รายการวันหยุดที่ตั้งไว้ (ระบบจัดการ)</h3>
                                <div className="space-y-2 max-h-[30vh] overflow-y-auto pr-1">
                                    {(db.bookings || []).filter(b => b.job_type === 'public_holiday' && String(b.status) !== 'cancelled').sort((a,b)=> new Date(b.date) - new Date(a.date)).map((h, i) => (
                                        <div key={i} className="flex justify-between items-center bg-red-50 p-2 rounded-lg border border-red-100">
                                            ...
                                            <button onClick={() => { if(window.confirm('ต้องการลบวันหยุดนี้?')) apiAction({action: 'delete_booking', id: h.id, user: user.username}, 'กำลังลบ...'); }} className="text-red-500 p-1.5 bg-white rounded shadow-sm border border-red-100"><Icons.X /></button>
                                        </div>
                                    ))}
