...
const App = () => {
        ...
        setPromptDialog({
            ...
            onSubmit: (reason) => {
                apiAction({ action: 'delete_booking', id: booking.id, user: user.username, reason: reason || 'ไม่ระบุเหตุผล' }, 'กำลังยกเลิกคิวงาน...').then(ok => {
                    if(ok) { setModal(null); setActionMenuId(null); showSlideToast('ยกเลิก/ลบสำเร็จ', 'success'); }
                });
            }
        });
    };
    ...
    useEffect(() => { 
        // 📍 [ข้อ 1: ป้องกันจอขาว 100%] ใช้ optional chaining ตรงนี้
    ...
    const handleBookingSubmit = async (e) => {
        ...
        // 📍 [ข้อ 1: ป้องกันจอขาว 100%] ใช้ optional chaining ตลอดเส้นทาง
        ...
        const payload = {
            ...
            ...data, tel: String(data.tel), area: finalArea, jobType: jobTypeSelection, product_line: finalProductLine,
        ...
        <div className="app-container">
                                                        ...
                                                        {cellTasks.map((task, tIdx) => {
                                                            ...
                                                            return (
                                                                <div key={task.id || tIdx} 
                                                                    ...
                                                                    {/* 📍 [ข้อ 3] ใส่คลาส auto-text ปรับฟอนต์ให้ห่อบรรทัด ไม่ล้นกรอบ */}
            ...
            {currentView === 'admin' && isAdmin && (
                <div className="page-view">
                        ...
                        <div className="animate-pop pb-10">
                            <div className="flex justify-between items-center mb-4">
                                <button onClick={() => {
                                    ...
                                    // 📍 [ข้อ 1 แก้จอขาว] ระบุว่าให้หน้าต่างมาจากแอดมินเพื่อความชัวร์
                    ...
                    {/* 📍 [ข้อ 2] ฟอร์มตั้งค่าวันลา พร้อมระบบจัดการ (ลบ) ด้านล่าง */}
                    {adminTab === 'leaves' && (
                                ...
                                <div className="space-y-2 max-h-[30vh] overflow-y-auto pr-1">
                                    {(db.bookings || []).filter(b => b.job_type === 'leave' && String(b.status) !== 'cancelled').map((h, i) => (
                                        <div key={i} className="flex justify-between items-center bg-yellow-50 p-2 rounded-lg border border-yellow-100">
                                            ...
                                            <button onClick={() => { if(window.confirm('ต้องการลบการตั้งค่าวันลานี้?')) apiAction({action: 'delete_booking', id: h.id, user: user.username}, 'กำลังลบ...'); }} className="text-red-500 p-1.5 bg-white rounded shadow-sm"><Icons.X /></button>
                                        </div>
                                    ))}
                    ...
                    {/* 📍 [ข้อ 2] ฟอร์มตั้งค่ากิจกรรมบริษัท พร้อมระบบจัดการ (ลบ) ด้านล่าง */}
                    {adminTab === 'events' && (
                        <div className="animate-pop space-y-4 pb-10">
                            <form onSubmit={async (e) => {
                                ...
                                <div className="space-y-2 max-h-[30vh] overflow-y-auto pr-1">
                                    {(db.bookings || []).filter(b => b.job_type === 'company_event' && String(b.status) !== 'cancelled').map((h, i) => (
                                        <div key={i} className="flex justify-between items-center bg-green-50 p-2 rounded-lg border border-green-100">
                                            ...
                                            <button onClick={() => { if(window.confirm('ต้องการลบการตั้งค่ากิจกรรมนี้?')) apiAction({action: 'delete_booking', id: h.id, user: user.username}, 'กำลังลบ...'); }} className="text-red-500 p-1.5 bg-white rounded shadow-sm"><Icons.X /></button>
                                        </div>
                                    ))}
                    ...
                    {/* 📍 [ข้อ 2] ฟอร์มตั้งค่าวันหยุดนักขัตฤกษ์ พร้อมระบบจัดการ (ลบ) ด้านล่าง */}
                    {adminTab === 'holidays' && (
                        <div className="animate-pop space-y-4 pb-10">
                            <form onSubmit={async (e) => {
                                e.preventDefault(); const fd = new FormData(e.target);
