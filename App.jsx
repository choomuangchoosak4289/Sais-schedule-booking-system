...
const App = () => {
    ...
    // 📍 State กิจกรรมบริษัท (ใหม่)
    const [eventStartDate, setEventStartDate] = useState('');
    const [eventEndDate, setEventEndDate] = useState('');

    // คำนวณจำนวนวันลาที่แท้จริง
    ...
    // 📍 คำนวณจำนวนวันกิจกรรมบริษัทที่แท้จริง
    const eventDates = useMemo(() => {
        if (!eventStartDate || !eventEndDate) return [];
        let start = new Date(eventStartDate);
        let end = new Date(eventEndDate);
        if (start > end) return [];
        ...
        <div className="app-container">
            ...
            {currentView === 'admin' && isAdmin && (
                <div className="page-view">
                    ...
                    {/* 📍 ฟอร์มตั้งค่าวันลา (เพิ่ม อื่นๆโปรดระบุ) */}
                    {adminTab === 'leaves' && (
                        <div className="animate-pop space-y-4 pb-10">
                            <form onSubmit={async (e) => {
                                ...
                                const finalLeaveName = leaveType === 'อื่นๆโปรดระบุ' ? customLeaveType : leaveType;

                                if (leaveType === 'อื่นๆโปรดระบุ' && !customLeaveType.trim()) {
                                    return setAlertMsg('กรุณาระบุประเภท/สาเหตุการลา');
                                }
                                ...
                                setConfirmDialog({
                                    ...
                                    onConfirm: async () => {
                                        ...
                                        const payload = { 
                                            ...
                                            job_type: 'leave', 
                                            site_name: finalLeaveName,
                                            equipment_no: `LEAVE_${Date.now()}`, 
                                            user: user.username 
                                        };
                                        const ok = await apiAction(payload, 'กำลังบันทึกวันลา...');
                                        if(ok) { 
                                            showSlideToast('เพิ่มวันลาสำเร็จ', 'success'); 
                                            setLeaveStartDate(''); setLeaveEndDate(''); setLeaveInspector(''); setCustomLeaveType('');
                                        }
                                    }
                                });
                                ...
                                <div className="space-y-3">
                                    ...
                                    <div>
                                        ...
                                        {/* 📍 ช่องกรอกข้อความเมื่อเลือก "อื่นๆโปรดระบุ" */}
                                        {leaveType === 'อื่นๆโปรดระบุ' && (
                                            <input 
                                                type="text" 
                                                value={customLeaveType} 
                                                onChange={e => setCustomLeaveType(e.target.value)} 
                                                placeholder="โปรดระบุสาเหตุการลา..." 
                                                required 
                                                className="mt-2 bg-white p-2 rounded-lg border border-yellow-300 w-full text-sm outline-none" 
                                            />
                                        )}
                                    </div>
                    ...
                    {/* 📍 ฟอร์มตั้งค่ากิจกรรมบริษัท */}
                    {adminTab === 'events' && (
                        <div className="animate-pop space-y-4 pb-10">
                            <form onSubmit={async (e) => {
                                e.preventDefault(); 
                                const fd = new FormData(e.target);
                                if (eventDates.length === 0) return setAlertMsg('ไม่มีวันที่สามารถตั้งค่าได้ (อาจตรงกับวันหยุดทั้งหมด หรือเลือกช่วงวันผิด)');
                                ...
                                setConfirmDialog({
                                    ...
                                    onConfirm: async () => {
                                        ...
                                        if(ok) { 
                                            ...
                                            setEventStartDate(''); setEventEndDate(''); e.target.reset();
                                        }
                                    }
                                });
                    ...
                    {adminTab === 'holidays' && (
                        <div className="animate-pop space-y-4 pb-10">
                            <form onSubmit={async (e) => {
                                ...
                                setConfirmDialog({
                                    msg: `ยืนยันตั้งค่าวัดหยุด: ${hName}?`,
                                    onConfirm: async () => {
                                        setConfirmDialog(null);
                                        const ok = await apiAction({ action: 'create_booking', date: fd.get('h_date'), inspector_name: 'SYSTEM_HOLIDAY', job_type: 'public_holiday', site_name: hName, equipment_no: `HLD_${Date.now()}`, user: user.username }, 'กำลังเพิ่มวันหยุด...');
                                ...
                                <div className="space-y-3">
                                    <div><label className="text-xs font-bold text-red-700">วันที่</label><input type="date" name="h_date" required className="bg-white p-2 rounded-lg border border-red-200 w-full text-sm outline-none" /></div>
                                    ...
                                    <button disabled={loadingMsg} className="w-full bg-red-600 text-white py-2.5 rounded-lg font-bold text-sm shadow-md">บันทึกวันหยุด</button>
                                </div>
                            </form>
                        </div>
                    )}
