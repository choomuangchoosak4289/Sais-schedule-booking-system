...
const App = () => {
    // 📍 ป้องกันจอขาว: ดึงค่าอย่างปลอดภัย (Optional Chaining)
    ...
    useEffect(() => {
        ...
        if(SCRIPT_URL) load();
        const timer = setInterval(() => { if (!modal && !showLogin && !showActivityModal && !alertMsg && !loadingMsg && SCRIPT_URL) fetchData(); }, 180000); 
        return () => clearInterval(timer);
    }, [modal, showLogin, showActivityModal, alertMsg, loadingMsg, SCRIPT_URL]);
                                            ...
                                            {!d.isEmpty && (db.inspectors || []).map((ins, idx) => {
                                                ...
                                                return (
                                                    <div key={idx} 
                                                        ...
                                                        {hasTask && (
                                                            <div draggable={isAdmin} onDragStart={(e) => handleDragStart(e, task.id)} className={`task-content ${cardTypeClass} ${areaClass} cursor-pointer`}>
                                                ...
                                                return filteredUsers.map((u, i) => (
                                                    <tr key={i} className="border-b border-slate-100">
                                                        ...
                                                        <td className="p-3 text-center flex justify-center gap-2">
                                                            {u.status === 'pending' && <button onClick={() => { if(window.confirm(`อนุมัติให้ ${u.username} ใช้งานระบบ?`)) apiAction({action: 'update_user_status', admin_user: user.username, target_user: u.username, new_status: 'approved'}, 'กำลังอนุมัติ...'); }} className="bg-green-500 text-white px-3 py-1 rounded-lg shadow-sm">อนุมัติ</button>}
                                                            {u.status === 'approved' && <button onClick={() => { if(window.confirm(`ระงับผู้ใช้ ${u.username} ไม่ให้เข้าสู่ระบบ?`)) apiAction({action: 'update_user_status', admin_user: user.username, target_user: u.username, new_status: 'blocked'}, 'กำลังระงับบัญชี...'); }} className="bg-red-500 text-white px-3 py-1 rounded-lg shadow-sm">บล็อก</button>}
                                                            {u.status === 'blocked' && <button onClick={() => { if(window.confirm(`ปลดบล็อกให้ ${u.username}?`)) apiAction({action: 'update_user_status', admin_user: user.username, target_user: u.username, new_status: 'approved'}, 'กำลังปลดบล็อก...'); }} className="bg-slate-500 text-white px-3 py-1 rounded-lg shadow-sm">ปลดบล็อก</button>}
                                                        </td>
                                                    </tr>
                                                ))
                                            })()}
                        ...
                        <div className="animate-pop space-y-4 pb-10">
                            <form onSubmit={async (e) => {
                                ...
                                <div className="space-y-3">
                                    ...
                                    <button disabled={loadingMsg} className="w-full bg-slate-800 text-white py-2 rounded-lg font-bold text-sm">บันทึกลงตาราง</button>
                                </div>
                            </form>
                            <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-200">
                                    ...
                                    {(db.logs || []).slice(0, logsLimit).map((log, i) => (
                                        <div key={i} className="bg-slate-50 p-3 rounded-xl border border-slate-200 shadow-sm">
                                            ...
                                            <div className="text-xs font-bold text-slate-800">
                                                <span className={`px-2 py-0.5 rounded text-[9px] mr-2 ${log.action === 'CREATE' || log.action === 'BOOKING' ? 'bg-green-100 text-green-700' : log.action === 'UPDATE' ? 'bg-blue-100 text-blue-700' : log.action === 'CANCEL' ? 'bg-orange-100 text-orange-700' : log.action === 'LOGIN' ? 'bg-purple-100 text-purple-700' : 'bg-slate-200 text-slate-700'}`}>
                                                    {log.action}
                                                </span>
                                            </div>
                        ...
                        <div className="text-sm text-slate-700 space-y-4 max-h-[60vh] overflow-y-auto pr-2">
                            ...
                            <div className="bg-blue-50 p-4 rounded-xl border border-blue-200">
                                ...
                                <ul className="list-decimal pl-4 text-xs space-y-2 text-blue-800">
                                    ...
                                    <li><b>กรอกข้อมูล:</b> เลือก Product Line และอัปโหลดรูปเอกสาร</li>
                                    <li><b>การจัดการ:</b> ไปที่แท็บ "งานของฉัน" กดจุด 3 จุด มุมขวาบนเพื่อแก้ไขหรือยกเลิกคิว (ระบบจะให้ระบุเหตุผลการยกเลิกเสมอ)</li>
                                </ul>
                            </div>
                        </div>
                    </div>
                        ...
                        <div className="p-6 overflow-y-auto">
                            ...
                            {modal.type === 'booking' ? (
                                ...
                                <div className="space-y-4">
                                    ...
                                    {/* 📍 [ข้อ 2] ระบบ Admin แยกปุ่ม แก้ไข / ยกเลิกชัดเจนในหน้า Detail */}
                                    {isAdmin && (
